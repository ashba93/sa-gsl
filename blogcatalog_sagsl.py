...
def train(dataset):
    ...
    optim = torch.optim.Adam(list(gnn.parameters()), lr=7e-2, weight_decay=1e-5)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        reg_term = 5e-1 * adj_density + 5e-1 * degree_penalty
...
if __name__ == "__main__":
    dataset = Dataset("blogcatalog", n_splits=1, split="random", cv=5)
    train(dataset)
    # exp = opengsl.ExpManager(train(dataset))
    # exp.run(n_runs = 1)